...
export default function ExamEditor() {
  ...
  return (
    <div className="space-y-6">
      <div className="card">
        <h2 className="title">Editor de Exámenes</h2>
        <p className="helper">Añade múltiples preguntas y define el % mínimo. Todo se guarda en LocalStorage.</p>
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <button className="px-3 py-1 rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-800" onClick={()=>window.scrollTo({ top: 0, behavior: 'smooth' })}>Nueva pregunta</button>
          </div>
        </div>
              ...
              {questions.map(q => (
                <li key={q.id} className="rounded-xl bg-slate-800/60 p-3 border border-slate-700">
                  ...
                  <div className="mt-3 flex gap-2">
                    <QuestionForm examType={type} initial={q} onSubmit={updateQuestion} onCancel={()=>{}} />
                    <button onClick={()=>removeQuestion(q.id)} className="px-3 py-2 h-fit rounded-xl border border-rose-700 text-rose-300 hover:bg-rose-900/30">Eliminar</button>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div className="mt-4 flex gap-2 items-center">
          <button onClick={saveExam} className="px-5 py-2 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-semibold">{isEditing ? 'Actualizar y guardar' : 'Guardar examen'}</button>
          {formError && <span className="error">⚠ {formError}</span>}
        </div>
      </div>

      <ExamList exams={exams} onEdit={(id) => {
